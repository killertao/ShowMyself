<template>
  <div>
    <p>data.name:<input type="text" v-model="data.name"/></p><!--v-data_model  如果是一个对象,会在第一次的时候给这个对象重新绑定Observer-->
    <p>data.msg:<input type="text" v-model="data.msg"/></p>
    <p>data.age:<input type="number" v-model="data.age"/></p>
    <p>data.name:{{data.name}} 不再对象里面的属性是否被监控：</p>
    <p>data.age:{{data.age}} 不再对象里面的属性是否被监控：</p>
...
<script>
  ...
  export default {
    data() {
      return {
        data: data//因为这里用得是一个对象，会将两个变量都改变
      }
    },
    methods: {
      clickBtn() {
